...
(function () {
  ...
  if (state.apiTemplate && (!state.apiTemplate.bodyText || state.apiTemplate.autoGenerated)) {
    // Synthetic templates (built from a token/profile, not from a real AINS POST)
    // hardcode the endpoint and omit fields the server needs, producing orphan
    // records or 400s. Discard them; only a genuinely captured POST is trusted.
  ...
  function updateCapturedToken(authHeader) {
    if (authHeader && authHeader.startsWith("Bearer ")) {
      ...
      // Note: do NOT auto-construct an apiTemplate from the token alone. A template
      // built with only `user` and no `student`/`school`/`class` relations still gets
      // HTTP 200 from Strapi but creates an orphan record invisible in AINS history
      // (v0.8.0 regression). Real relations must come from a genuine capture: either
      // the profile-response intercept below, or a manually submitted real record.
      saveState();
      renderApiStatus();
      syncWorkerCursor();
  ...
  function captureTemplate(url, method, headers, body) {
    ...
    renderApiStatus();
    if (capture.active) {
      // Seeding submission from the semi-auto capture flow just went through:
      // this is a genuine website-built POST. Finish and switch to API mode.
      finishSemiAutoCapture();
      return;
    }
  ...
  function applyProfileData(profile) {
    // Only ever backfill missing relations onto a *genuinely captured* template.
    // We never synthesize a template from profile data alone: the real AINS POST
    // carries the correct endpoint, provider signature and hidden fields that a
    // hand-built body cannot reproduce (that path caused orphan records / 400s).
    if (!profile || !state.apiTemplate || state.apiTemplate.autoGenerated) return;
    const payload = parseJsonOrNull(state.apiTemplate.bodyText);
    const data = payload?.data;
    if (data && (!data.student || !data.school)) {
      data.student = data.student || profile.student;
      data.school = data.school || profile.school;
      data.class = data.class || profile.class;
      state.apiTemplate.bodyText = JSON.stringify(payload);
      state.apiTemplate.payload = payload;
      saveState();
      console.log("NILAM API Assistant: Backfilled missing relations on captured template ->", profile);
    }
  }

  // ===========================================================================
  // Semi-automatic real-template capture.
  //
  // The reliable way to get a submittable template is to let the AINS website's
  // own code build one genuine POST (correct endpoint, provider signature, and
  // all relation/hidden fields), then capture it. This flow autofills the native
  // "add record" form with a fresh unused book from our pool so the seeding
  // submission is a real, non-wasted NILAM record. The existing fetch/XHR
  // interception (captureTemplate) stores the resulting real POST as the
  // template; afterwards fast API submits reuse it.
  // Form-fill helpers are ported from the proven nilam-assistant.user.js engine.
  // ===========================================================================
  ...
  function findDateInput() {
    const ok = (el) => isUsableElement(el) && !isInsidePanel(el);
    const directSelectors = [
      'input[type="date"]', 'input[id*="date" i]', 'input[id*="tarikh" i]',
      'input[name*="date" i]', 'input[name*="tarikh" i]', 'input[placeholder*="date" i]',
      'input[placeholder*="tarikh" i]', 'input[aria-label*="date" i]', 'input[aria-label*="tarikh" i]'
    ];
    for (const selector of directSelectors) {
      const found = Array.from(document.querySelectorAll(selector)).find(ok);
    ...
    const visibleInputs = Array.from(document.querySelectorAll("input")).filter(ok);
  ...
  function fillDate(isoDate) {
    ...
    const dateValue = dateValueForInput(dateInput, isoDate);
    ...
    ["keydown", "keypress", "keyup"].forEach((type) =>
      dateInput.dispatchEvent(new KeyboardEvent(type, { bubbles: true, cancelable: true, key: "Enter", code: "Enter" })));
  ...
  function selectDropdownByText(selectIndex, targetText) {
    ...
    const opt = Array.from(selectEl.options).find((option) => option.textContent.trim().includes(targetText));
    if (!opt) return false;
  ...
  function scrollToBottomHard() {
    const doScroll = () => {
      window.scrollTo({ top: document.documentElement.scrollHeight || document.body.scrollHeight, behavior: "auto" });
      document.documentElement.scrollTop = document.documentElement.scrollHeight;
      document.body.scrollTop = document.body.scrollHeight;
      document.querySelectorAll("ion-content, .ion-content-scroll-host, .scroll-content, [class*='content-scroll'], main, [role='main']")
        .forEach((el) => { if (el.scrollToBottom) el.scrollToBottom(0); else el.scrollTop = el.scrollHeight; });
  ...
  function scrollToBottomOnce(key) {
    if (capture.lastScrolledKey === key) return;
    capture.lastScrolledKey = key;
    scrollToBottomHard();
  }

  function clickButtonByText(text) {
    const btn = Array.from(document.querySelectorAll("button, span, div, p"))
      .find((el) => !isInsidePanel(el) && (el.textContent || "").trim() === text);
    if (!btn) return false;
  ...
  function nilamLanguageText(language) {
  ...
  function bookByKey(key) {
    return state.books.find((book) => bookKey(book) === key) || null;
  }

  function captureFillPage1(book) {
    if (capture.filledPage1) return;
    capture.filledPage1 = true;
    fillDate(normalizeIsoDate(state.selectedDate));
    setValue(document.getElementById("title"), book.title);
    setValue(document.getElementById("noOfPage"), Number(book.pages || 0));
    setValue(document.getElementById("isbn"), formatIsbn(book.isbn));
    ...
    selectDropdownByText(1, nilamLanguageText(book.language));
    setStatus("捕获模式：第 1 页已填。正在前往下一页...");
    setTimeout(() => clickButtonByText("Seterusnya"), 700);
  }

  function captureFillPage2(book) {
    if (!capture.filledPage2) {
      capture.filledPage2 = true;
      setValue(document.getElementById("summary"), book.rumusan);
      setValue(document.getElementById("review"), book.lesson);
      setStatus("捕获模式：第 2 页已填。正在点五星、滚动到提交...");
      setTimeout(() => { forceClickFifthStar(); scrollToBottomHard(); }, 500);
    }
    forceClickFifthStar();
  }

  function captureFillTick() {
    if (!capture.active) { stopSemiAutoCapture(); return; }
    if (Date.now() - capture.startedAt > 5 * 60 * 1000) {
      stopSemiAutoCapture();
      setStatus("捕获模式已超时（5 分钟）。如仍需捕获请重新点击「① 半自动捕获」。");
      return;
    }
    const book = bookByKey(capture.bookKey);
    if (!book) { stopSemiAutoCapture(); return; }
    const formBook = bookForApi(book);
    if (document.getElementById("title")) { captureFillPage1(formBook); return; }
    if (document.getElementById("summary")) { captureFillPage2(formBook); return; }
    const buttons = Array.from(document.querySelectorAll("button, span, div, p"));
    const hasHantar = buttons.some((el) => { const t = (el.textContent || "").trim(); return t === "Hantar" || t === "Simpan"; });
    const hasSeterusnya = buttons.some((el) => (el.textContent || "").trim() === "Seterusnya");
    if (hasHantar) { scrollToBottomOnce("hantar"); setStatus("捕获模式：请点击官方「Hantar / Simpan」提交这一本，脚本会自动捕获真实模板。"); }
    else if (hasSeterusnya) scrollToBottomOnce("seterusnya");
  }

  function startSemiAutoCapture() {
    if (!state.userId) { setStatus("请先在 AINS 登录（脚本需先捕获到登录令牌）。"); return; }
    applyFilters();
    const book = currentBook();
    if (!book) { setStatus("题库里没有可用的未读书。请先点「Refresh Unused Books」或「同步记录」。"); return; }
    if (isCurrentBookDuplicate()) {
      const skipped = skipUsedBookSelection();
      if (!skipped) { setStatus("当前列表没有未读书可供播种。请先刷新题库。"); return; }
    }
    const seedBook = currentBook();
    capture.active = true;
    capture.bookKey = bookKey(seedBook);
    capture.filledPage1 = false;
    capture.filledPage2 = false;
    capture.lastScrolledKey = "";
    capture.startedAt = Date.now();
    if (capture.loopId) clearInterval(capture.loopId);
    capture.loopId = setInterval(captureFillTick, 800);
    renderApiStatus();
    setStatus(`捕获模式已开启。请在 AINS 点击底部「＋」新增记录，脚本会自动把《${seedBook.title}》填进官方表单；填完后点官方「Hantar / Simpan」提交即可自动捕获。`);
    try {
      alert(`半自动捕获真实模板\n\n即将用这本未读书播种一次真实提交：\n《${seedBook.title}》\n\n步骤：\n1) 点击 AINS 底部的「＋」打开新增记录表单。\n2) 脚本会自动逐页填入内容并点五星。\n3) 最后你亲自点官方「Hantar / Simpan」提交。\n4) 脚本自动捕获这条真实请求作为模板，之后即可用「API 提交」秒交。\n\n若某个字段没填上，可手动补，只要最终成功提交即可。`);
    } catch (e) { /* alert blocked, status line already shown */ }
  }

  function stopSemiAutoCapture() {
    capture.active = false;
    if (capture.loopId) { clearInterval(capture.loopId); capture.loopId = null; }
  }

  function finishSemiAutoCapture() {
    if (!capture.active) return;
    const book = bookByKey(capture.bookKey);
    stopSemiAutoCapture();
    if (book) {
      const titles = new Set(state.submittedTitles || []);
      titles.add(String(book.title).trim().toLowerCase());
      state.submittedTitles = Array.from(titles);
      const isbns = new Set(state.submittedIsbns || []);
      const isbnClean = String(book.isbn || "").replaceAll("-", "").replaceAll(" ", "").trim().toLowerCase();
      if (isbnClean) isbns.add(isbnClean);
      state.submittedIsbns = Array.from(isbns);
      state.todaySubmitCount = (state.todaySubmitCount || 0) + 1;
      state.lastSubmitTime = Date.now();
      advanceBookCursor(book.language);
    }
    saveState();
    applyFilters();
    renderApiStatus();
      ...
      if (response.ok) {
        ...
        if ((url.includes("users/me") || url.includes("student-informations")) && String(method).toUpperCase() === "GET") {
          try {
            const clone = response.clone();
            const data = await clone.json();
            applyProfileData(findProfileData(data, state.userId));
          } catch (e) {
            console.error("Failed to parse intercepted Fetch profile response", e);
          }
        }
  ...
  async function submitApi() {
    ...
    if (!state.apiTemplate?.bodyText || state.apiTemplate.autoGenerated) {
      setStatus("尚无真实模板：请点「① 半自动捕获真实模板」提交 1 本以捕获，再用 API 快速提交。");
  ...
  function renderApiStatus() {
    ...
    const status = tokenStatus();
    if (el) {
      const relations = capturedDataTemplate();
      const hasRealTemplate = state.apiTemplate?.bodyText && relations.student && relations.school;
      el.innerHTML = capture.active
        ? `<span style="color:#8b5cf6;font-weight:700;">捕获模式进行中…</span><br><span style="font-size:10px;color:#6b7280;">请在 AINS 打开新增记录表单并提交这一本 | v${SCRIPT_VERSION}</span>`
        : hasRealTemplate
          ? `真实模板：<span style="color:#10b981;font-weight:700;">已捕获</span><br><span style="font-size:10px;color:#6b7280;">User ID: ${state.userId || "等待中"} | ${status.label} | v${SCRIPT_VERSION}</span>`
          : state.authHeader
            ? `<span style="color:#d97706;font-weight:700;">需捕获真实模板</span><br><span style="font-size:10px;color:#6b7280;">User ID: ${state.userId || "等待中"} | 请点「① 半自动捕获真实模板」提交 1 本以捕获 | v${SCRIPT_VERSION}</span>`
            : `<span style="color:#ef4444;font-weight:700;">未捕获登录令牌</span><br><span style="font-size:10px;color:#6b7280;">请先在 AINS 正常登录 | v${SCRIPT_VERSION}</span>`;
    }
  ...
  function createPanel() {
    ...
    try {
      ...
      <div class="nia-body" id="nia-body" style="display: ${state.collapsed ? "none" : "block"};">
        ...
        <button id="nia-capture" type="button" class="nia-btn nia-submit" style="width:100%;margin-top:8px;">① 半自动捕获真实模板（填一本真书）</button>
    ...
    `;
      ...
      panel.addEventListener("click", async (event) => {
        ...
        if (button.id === "nia-capture") startSemiAutoCapture();
        ...
        if (button.id === "nia-clear-api") {
          ...
          renderApiStatus();
          renderBookSelect();
          setStatus("凭证与模板已清除。请在 AINS 正常登录后，点「① 半自动捕获真实模板」重新捕获。");
        }
      });
