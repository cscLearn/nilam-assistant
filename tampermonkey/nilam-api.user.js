...
(function () {
  ...
  function applyProfileData(profile) {
    if (!profile) return;
    if (state.apiTemplate && !state.apiTemplate.autoGenerated) {
      // A genuine captured template already has real relations; don't clobber it,
      // just backfill any relation field it might be missing.
      const payload = parseJsonOrNull(state.apiTemplate.bodyText);
    ...
    saveState();
    renderApiStatus();
    setStatus("凭证已静默捕获（自动提取学生关联）！已就绪，可开始同步。");
  }
  ...
  async function submitApi() {
    ...
    if (!base.student || !base.school) {
      setStatus("提交拦截：缺少学生/学校关联，提交会变成孤儿记录（AINS 不会显示）。请稍等自动同步，或在 AINS 官方页面手动交一次以捕获完整凭证。");
  ...
  function renderApiStatus() {
    ...
    if (el) {
      ...
      const missingRelations = state.apiTemplate?.bodyText && (!relations.student || !relations.school);
      el.innerHTML = missingRelations
        ? `<span style="color:#d97706;font-weight:700;">缺少学生关联</span><br><span style="font-size:10px;color:#6b7280;">User ID: ${state.userId || "等待中"} | 提交会变孤儿记录，AINS 不会显示；请稍等自动同步或手动交一次 | v${SCRIPT_VERSION}</span>`
        : state.apiTemplate?.bodyText
